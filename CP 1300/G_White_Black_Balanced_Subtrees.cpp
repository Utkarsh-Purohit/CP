...
int fun(int node, const vector<vector<int>> &childs, const string &s, int &count)
{
    int ans = (s[node - 1] == 'B') ? -1 : 1 ;
    for (int val : childs[node])
    {
        ans = ans + fun(val, childs, s, count) ;
    }
    if (ans == 0) count++ ;
    return ans ;
}
