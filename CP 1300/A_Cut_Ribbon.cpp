...
int fun(int n, int a, int b, int c, vector<int>& dp) 
{
    if (n == 0) return 0 ;
    if (n < 0) return -1 ;  
    if (dp[n] != -2) return dp[n];
    int c1 = fun(n - a, a, b, c, dp) ;
    int c2 = fun(n - b, a, b, c, dp) ;
    int c3 = fun(n - c, a, b, c, dp) ;
...
int main() 
{
    ...
    // while(T--)
    {
        int n, a, b, c ; take4(n,a,b,c) ;
        vii dp(n + 1, -2) ;
        int ans = fun(n, a, b, c, dp) ;
        give(ans);
    }
}
