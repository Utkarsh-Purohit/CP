        ...
        else
        {
            repi(0 , s.size()-2)
            {
                if(s[i] == s[i+1])
                {
                    ...
                    {
                        if(s[i]=='z') s[i+1] = 'a' ;
                        else s[i+1] = char(int(s[i])+1) ;
                        i++ ;
                    }
                    else
                    {
                        int x = int(s[i]) , y = int(s[i+2]) ;
                        if(abs(x-y)==1)
                        {
                            if(char(max(x,y))=='z' && char(min(x,y))=='a') s[i+1] = 'b' ;
                            else if(char(max(x,y)) == 'z') s[i+1] = 'a' ;
                            else s[i+1] = char(max(x,y)+1) ;
                        }
                        else s[i+1] = char(min(x,y)+1) ;
                        i++ ;
                    }
                }
            }
            ...
            {
                if(s[s.size()-2] == 'z') s[s.size()-1] = 'a' ;
                else s[s.size()-1] = char(int(s[s.size()-2]+1)) ;
            }
            give(s) ;
        }
    }
