...
int main() 
{
    ...
    while(T--)
    {
        ...
        for(int i = 1 ; i < n ; i++) 
        {
            if(v[i] != v[i-1]) 
            {
                pair<int,int> p = {i , v[i-1]};
                if(bs.empty() || (bs.back() != p && bs.back().second != v[i-1])) bs.pb({i , v[i-1]});
                bs.pb({i+1 , v[i]});
            }
        }
        for(int i = 0 ; i < q ; i++)
        {
            int l , r; take2(l,r);
            auto comp = [](const pii &a, const int &val) 
            {
                return a.first < val;
            };
            int ind = lower_bound(bs.begin(), bs.end(), l, comp) - bs.begin();
            if(ind == bs.size())  cout << "-1 -1" << endl;
            else 
            {
                if(bs[ind].first == l) 
                {
                    if(ind + 1 >= bs.size() || bs[ind + 1].first > r) cout << "-1 -1" << endl;
                    else cout << l << " " << bs[ind + 1].first << endl;
                    
                } 
                else 
                {
                    if(bs[ind].first > r) cout << "-1 -1" << endl;
                     else cout << l << " " << bs[ind].first << endl;
