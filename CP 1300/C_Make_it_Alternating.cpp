...
{
    ll product = 1;
    for (int i = 1; i <= n; i++) product = (product * i) % 998244353 ; return product;
}
...
int main() 
{
    ...
    while(T--)
    {
        ...
        while(i < n - 1)
        {
            ...
            if (count > 1) v.pb(count);
            i++;
        }
        debug(v) ;
        if(v.size() == 0) cout<<"0"<<" "<<'1'<<endl ;
        else
        {
            int count = 0 ; 
            for(int i = 0 ; i < v.size() ; i++) count = count + v[i] - 1 ;
            ll modulo = 998244353 ;
            ll ans = 1 ;
            ...
            ans = (ans * factorial(count)) % modulo ;
            cout<<count<<" "<<ans<<endl ;
        }
    }
}
