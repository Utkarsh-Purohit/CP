...
void dfs(int node, int color, vector<int>& hash, const vector<vector<int>>& adj) 
{
    hash[node] = color;
...
int main() 
{
    ...
    // while(T--)
    {
        ...
        repi(1,n+1)
        {
            if (hash[i] == 0) s1.insert(i);
            else if (hash[i] == 1) s2.insert(i);
        }
        ll ans = (s1.size() * s2.size()) - (n-1) ;
        give(ans) ;
    }
}
